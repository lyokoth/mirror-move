...
import { cleanPokemonName, fetchPokemonSprite, fetchTypeHex, returnPokemonType } from '../utils/pokeUtils.js';
...
const formatDisplayData = (pokemon) => {
    ...
    if (pokemon.evs) {
        ...
        const evString = ['HP', 'Atk', 'Def', 'SpA', 'SpD', 'Spe']
            .filter(stat => !isNaN(evs[stat.toLowerCase()]))
            .map(stat => `${evs[stat.toLowerCase()]} ${stat}`)
            .join(' / ');
        str += `\`${evString}\`\n`;
